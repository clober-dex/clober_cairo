...
# Deployment script for BookViewer
# Example: ./deploy_book_viewer.sh --network dev 0x1234...5678 0x1234...5678
...
if [[ "${#args[@]}" -ne 2 ]]; then
    echo "Error: Arguments missing. Usage: ./deploy_book_viewer.sh --network dev 0x1234...5678 0x1234...5678"
    exit 1
fi
...
command_output=$(starkli declare ./target/dev/clober_cairo_BookViewer.contract_class.json --rpc=$rpc --compiler-version=2.7.1 --account $account $key_command)

from_string="Class hash declared:"
class_hash="${command_output#*$from_string}"

# Deploy the contract using the extracted class hash
starkli deploy $class_hash ${args[0]} ${args[1]} --rpc=$rpc --account $account $key_command
