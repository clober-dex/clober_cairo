...
use openzeppelin_testing::constants::{ZERO, OWNER, NAME, SYMBOL};
...
fn setup() -> (
    ...
    let mut calldata = array![];
    calldata.append_serde(book_manager);
    let claim_router = utils::declare_and_deploy("ClaimRouter", calldata);
...
#[test]
fn test_success() {
    let (bm, open_router, make_router, take_router, claim_router, base, quote) = setup();
    ...
    let before_order_info = bm.get_order(order_id);
    ...
    erc721.approve(claim_router.contract_address, order_id.into());

    let mut spy = spy_events();
    spy.drop_all_events();
    cheat_caller_address(claim_router.contract_address, OWNER(), CheatSpan::TargetCalls(1));
    let claimed_amount = claim_router.claim(order_id, ArrayTrait::new().span());
    spy.assert_only_event_claim(bm.contract_address, order_id, take_unit);

    let order_info = bm.get_order(order_id);

    assert_eq!(claimed_amount, tick.quote_to_base(take_unit.into() * key.unit_size.into(), false));
    ...
    assert_eq!(order_info.open, before_order_info.open);
    assert_eq!(order_info.claimable, before_order_info.claimable - take_unit);
    assert_eq!(erc721.balance_of(OWNER()), 1);
    assert_eq!(erc721.owner_of(order_id.into()), OWNER());
}

#[test]
fn test_claim_should_burn_with_zero_pending_order() {
    let (bm, open_router, make_router, take_router, claim_router, base, quote) = setup();
    let key = valid_key(base.contract_address, quote.contract_address);
    open_router.open(key, ArrayTrait::new().span());
    let make_unit = 10000;
    ...
    cheat_caller_address(take_router.contract_address, OWNER(), CheatSpan::TargetCalls(1));
    take_router.take(TakeParams { key, tick, max_unit: make_unit }, ArrayTrait::new().span());
    ...
    cheat_caller_address(claim_router.contract_address, OWNER(), CheatSpan::TargetCalls(1));
    let claimed_amount = claim_router.claim(order_id, ArrayTrait::new().span());
    spy.assert_event_claim(bm.contract_address, order_id, make_unit);

    let order_info = bm.get_order(order_id);

    assert_eq!(claimed_amount, tick.quote_to_base(make_unit.into() * key.unit_size.into(), false));
    assert_eq!(quote.balance_of(OWNER()), before_quote_balance);
    assert_eq!(
        base.balance_of(OWNER()),
...
fn test_claim_nonexistent_order() {
    let (_, open_router, _, _, claim_router, base, quote) = setup();
    let key = valid_key(base.contract_address, quote.contract_address);
    open_router.open(key, ArrayTrait::new().span());

    cheat_caller_address(claim_router.contract_address, OWNER(), CheatSpan::TargetCalls(1));
    claim_router.claim(123, ArrayTrait::new().span());
}
